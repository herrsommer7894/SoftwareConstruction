	...
	#	$line =~ /print\s*[\("']*(.+)\\*\n*[\)"']/;
		...
		if ($var =~ /\$\w+|\@\w+/){
			...
			if (exists $variables{$check}){
	       			$line =~s/print\s*"(.*)\\n"[\s;]*/print($var)\n/;
			} elsif ($var =~ /join\s*\(|eval\s*\(|split\s*\(|ARGV|pop|push|reverse|unshift|shift/){
				$line =~ s/print.*["']\n/print\($var\)\n/g;
			} elsif ($line =~ /\+|\-|\*{1,2}|\/|\%/){
...
sub variable_plpy_syntax{
	...
	if($line =~ /\@(\w+)/){
		...
		if($line =~ /scalar\(\s*\@(\w+)\s*\)/){
			$var = $1;
			$line =~ s/scalar\(\s*\@\w+\s*\)/len($var)/;
		}

		#fix push, pop, unshift, shift, from array
